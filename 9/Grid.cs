...
public class Grid<T>
{
    ...
    public T[] GetNeighbors1D(T[,] grid, int x, int y, int n)
    {
        ...
        for (int i = 0; i < neighborSquareSideLength; i++)
        {
            for (int j = 0; j < neighborSquareSideLength; j++)
            {
                neighbors[j * (neighborSquareSideLength + i)] = FancyIndex(grid, neighborSquareStart.x + j, neighborSquareStart.y + i);
            }
        }
    ...
    {
        return Values[x % gridWidth][y % gridHeight];
    }

    public T FancyWrapIndex(T[,] grid, int x, int y)
    {
        return Values[x % grid.GetLength(1)][y % grid.GetLength(0)];
    }
